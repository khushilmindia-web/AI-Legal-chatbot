...
def has_case_corpus() -> bool:
    return DATA_DIRECTORY.exists() and any(DATA_DIRECTORY.rglob("*.txt"))


def case_rag_answer(query: str, state: str | None = None, matter_type: str | None = None) -> dict:
    if not has_case_corpus():
        return {
            "answer": "",
            "confidence": 0.0,
            "chunks": [],
    ...
    try:
        ...
        results = prioritized[:TOP_K]
    except Exception as e:
        logger.error("Case-law RAG retrieval error: %s", e)
...
Rules:
1. Answer only from the provided case-law context.
2. Summarize the ratio or principle relevant to the query.
...
User query: {query}
State context: {state or 'Not specified'}
